...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "POST" && req.url === "/api/admin/test-cleanup") {
      ...
      try {
        ...
        const result = await client.query(
          `SELECT id, user_id, amount, type, status, reference
           FROM wallet_transactions
           WHERE reference = $1
           FOR UPDATE`,
          ["NH-MTKHH566-4B5A4BEB"]
        );

        if (result.rows.length === 0) {
          await client.query("ROLLBACK");
          sendJSON(res, 404, { error: "Test transaction not found" });
          return;
        }
        ...
        if (walletResult.rows.length === 0) {
          await client.query("ROLLBACK");
          sendJSON(res, 409, {
            error: "Wallet reversal failed; wallet was not changed"
          });
          return;
        }
        ...
        const deleteResult = await client.query(
          ...
          [tx.id, "NH-MTKHH566-4B5A4BEB"]
        );

        if (deleteResult.rows.length !== 1) {
          await client.query("ROLLBACK");
          sendJSON(res, 409, {
            error: "Transaction deletion failed; wallet reversal was rolled back"
          });
          return;
        }
        ...
        sendJSON(res, 200, {
          message: "Test ₦100 deposit reversed and removed successfully",
          deletedTransaction: deleteResult.rows[0],
          wallet: walletResult.rows[0].wallet
        });
      } catch (error) {
        await client.query("ROLLBACK");
        console.error("Test cleanup error:", error);
        sendJSON(res, 500, { error: "Test cleanup failed" });
      } finally {
        client.release();
      }
